...
<dt>シンプルさを優先する</dt>
<dd>
  複雑な抽象化を避け、多くの開発者がコードを読み解き手を加えられることを重視します。プログラマー的な正しさを追求して複雑にすることよりも、理解しやすさを優先します。
</dd>
<dt>後付け可能な設計</dt>
<dd>
...
<dl>
<dt>自動テスト</dt>
<dd>axe DevToolsを使用した自動アクセシビリティテストを実施します。</dd>
<dt>専門家レビュー</dt>
...
原則として、組み込みの多言語サポートは行いません。ただし、コンポーネントと言語の密結合を避けるため、以下の方針を採用します。

- JSやCSSにインラインで言語依存情報をを含めない
- 言語依存のテキストは通常のHTML要素として記述する
...
デフォルトで指定するブレイクポイントは48rem（768px）とし、モバイルファーストのアプローチを採用します。
