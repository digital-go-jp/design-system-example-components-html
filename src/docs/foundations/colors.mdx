...
export const keyColorData = Object.entries(tokens.Color.Key).map(([level, token]) => ({
  level,
  hex: token.$value,
  cssVar: generateCSSVarName(token.path)
}));
...
<div className="color-section">
  ...
  <div className="color-group">
    <h3 id="group-Key-name">Key</h3>
    <dads-scroll-shadow style={{"--scroll-shadow-padding": "calc(16 / 16 * 1rem)"}}>
      <div className="color-grid">
        {keyColorData.map((color, index) => (
          <div key={index} className="color-card">
            <div className="color-info">
              <h4 id={`color-key-${color.level}-name`} className="color-level">{color.level}</h4>
            </div>
            <div
            ...
            <CopyButton text={`var(${color.cssVar})`} label="CSS変数をコピー" ariaDescribedby={`group-Key-name color-key-${color.level}-name`} />
            <pre className="color-css"><code>{color.hex}</code></pre>
            <CopyButton text={color.hex} label="色コードをコピー" ariaDescribedby={`group-Key-name color-key-${color.level}-name`} />
          </div>
        ))}
      </div>
    </dads-scroll-shadow>
