...
カルーセルの基底要素は`<dads-carousel>`カスタム要素です。この要素には`role="region"`を付与し、適切なラベルを設定する必要があります。ラベルの付け方は、見出しの有無によって異なります。
...
見出しを使用しない場合は、基底要素の`aria-label`属性で直接ラベルを指定します。推奨値は「スライドショー」です。
...
`.dads-carousel__heading`要素は任意で追加できる見出しです。カルーセルの内容を説明する見出しを表示する場合に使用します。

```html
<h2 id="carousel-heading" class="dads-carousel__heading">開催中のイベント</h2>
```

- 見出しのレベル（`<h2>`、`<h3>`など）は、ページの文書構造に応じて適切なものを選択してください
- 見出しには一意のIDを付与し、基底要素の`aria-labelledby`属性で参照してください
...
スライドが1枚のみの場合は、シンプルなマークアップを使用できます。この場合、カルーセルのナビゲーション機能は不要となるため、依存するファイルが異なります。

### 必要なファイルを読み込む

シングルの場合は以下のファイルのみを読み込みます。

- `carousel-single.css`

### マークアップ

シングルの場合は`<dads-carousel>`カスタム要素ではなく、シンプルな`<div>`要素を使用します。

```html
<div class="dads-carousel-single">
  <span class="dads-carousel-single__link">
    <img class="dads-carousel-single__image" src="{画像のURL}" alt="{画像の代替テキスト}" width="{画像の幅}" height="{画像の高さ}">
  </span>
</div>
```

画像にリンクを設定する場合は、`<span>`要素を`<a>`要素に置き換えてください。

```html
<div class="dads-carousel-single">
  <a class="dads-carousel-single__link" href="{リンクURL}">
    <img class="dads-carousel-single__image" src="{画像のURL}" alt="{画像の代替テキスト}" width="{画像の幅}" height="{画像の高さ}">
  </a>
</div>
```
...
```html
<img src="image-1.webp" srcset="image-1@2x.webp 2x" alt="{画像の代替テキスト}" width="{画像の幅}" height="{画像の高さ}">
```

- `src`属性には通常解像度（1x）の画像を指定します
- `srcset`属性には、2倍解像度の画像に` 2x`を付けて指定します
- 画像の幅および高さには、通常解像度（1x）の画像のサイズを指定します

この指定はカルーセル内のすべての`<img>`要素（メインパネル、ネクストパネル、すべてのスライド内の画像）に適用してください。
